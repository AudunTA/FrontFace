...
window.onresize = function () {
  if (window.innerWidth <= 1000)
    profileContainer.classList.remove("sticky-top");
...
async function findFriends() {
  ...
  try {
    const response = await fetch(
      `${url}/social/profiles?_followers=true&limit=100`,
      options
    );
    const result = await response.json();
    console.log(result);
      ...
      if (!listFollowers.includes(result[i].name)) {
        ...
        <div class="d-flex border-bottom m-1">
          ...
          <div class="d-flex flex-column">
            <p class="mt-3">${result[i].name}</p>
            <button class="btn btnConnect" id="${result[i].name}">follow</button>
          </div>
        </div>`;
